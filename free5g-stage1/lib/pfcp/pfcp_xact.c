...
static int pfcp_xact_initialized = 0;
...
index_declare(pfcp_xact_pool, pfcp_xact_t, SIZE_OF_PFCP_XACT_POOL);

static pfcp_xact_stage_t pfcp_xact_get_stage(c_uint8_t type, c_uint32_t sqn);
static status_t pfcp_xact_delete(pfcp_xact_t *xact);

status_t pfcp_xact_init(tm_service_t *tm_service,
        c_uintptr_t response_event, c_uintptr_t holding_event)
{
    d_assert(pfcp_xact_initialized == 0, return CORE_ERROR,
        "PFCP Transaction already has been initialized");

    index_init(&pfcp_xact_pool, SIZE_OF_PFCP_XACT_POOL);
    ...
    pfcp_xact_initialized = 1;

    return CORE_OK;
}

status_t pfcp_xact_final(void)
{
    d_assert(pfcp_xact_initialized == 1, return CORE_ERROR,
        "PFCP Transaction context already has been finalized");

    if (pool_used(&pfcp_xact_pool))
        d_error("%d not freed in pfcp_xact_pool[%d] of PFCP Transaction",
                pool_used(&pfcp_xact_pool), pool_size(&pfcp_xact_pool));
    d_trace(15, "%d not freed in pfcp_xact_pool[%d] of PFCP Transaction\n",
            pool_used(&pfcp_xact_pool), pool_size(&pfcp_xact_pool));
    index_final(&pfcp_xact_pool);

    pfcp_xact_initialized = 0;

    return CORE_OK;
}

pfcp_xact_t *pfcp_xact_local_create(
        pfcp_node_t *gnode, pfcp_header_t *hdesc, pkbuf_t *pkbuf)
{
    ...
    xact->xid = NEXT_ID(g_xact_id, PFCP_MIN_XACT_ID, PFCP_MAX_XACT_ID);
    xact->gnode = gnode;

    if (g_response_event)
    {
        xact->tm_response = timer_create(g_tm_service,
                g_response_event, PFCP_T3_RESPONSE_DURATION);
    ...
    if (g_holding_event)
    {
        xact->tm_holding = timer_create(g_tm_service,
                g_holding_event, PFCP_T3_DUPLICATED_DURATION);
    ...
    list_append(xact->org == PFCP_LOCAL_ORIGINATOR ?  
            &xact->gnode->local_list : &xact->gnode->remote_list, xact);

    rv = pfcp_xact_update_tx(xact, hdesc, pkbuf);
    d_assert(rv == CORE_OK, return NULL, "Update Tx failed");

    d_trace(15, "[%d] %s Create  peer [%s]:%d\n",
            xact->xid,
            xact->org == PFCP_LOCAL_ORIGINATOR ? "LOCAL " : "REMOTE",
...
pfcp_xact_t *pfcp_xact_remote_create(pfcp_node_t *gnode, c_uint32_t sqn)
{
    char buf[CORE_ADDRSTRLEN];
    pfcp_xact_t *xact = NULL;
    ...
    xact->org = PFCP_REMOTE_ORIGINATOR;
    xact->xid = PFCP_SQN_TO_XID(sqn);
    xact->gnode = gnode;
...
void pfcp_xact_delete_all(pfcp_node_t *gnode)
{
    pfcp_xact_t *xact = NULL;
...
status_t pfcp_xact_update_tx(pfcp_xact_t *xact,
        pfcp_header_t *hdesc, pkbuf_t *pkbuf)
{
    char buf[CORE_ADDRSTRLEN];
    pfcp_xact_stage_t stage;
    ...
    stage = pfcp_xact_get_stage(hdesc->type, xact->xid);
    if (xact->org == PFCP_LOCAL_ORIGINATOR)
    {
        switch(stage)
        {
    ...
    else if (xact->org == PFCP_REMOTE_ORIGINATOR)
    {
        switch(stage)
        {
            case PFCP_XACT_INITIAL_STAGE:
    ...
    {
        pkbuf_header(pkbuf, PFCP_HEADER_LEN);
        hdr_len = PFCP_HEADER_LEN;
    }
    else
    {
        pkbuf_header(pkbuf, PFCP_HEADER_LEN-PFCP_SEID_LEN);//$ no SEID
        hdr_len = PFCP_HEADER_LEN-PFCP_SEID_LEN;
    }
    ...
    {
        h->seid_p = 0;
        h->sqn_only = PFCP_XID_TO_SQN(xact->xid);
    }
...
status_t pfcp_xact_update_rx(pfcp_xact_t *xact, c_uint8_t type)
{
    status_t rv = CORE_OK;
    char buf[CORE_ADDRSTRLEN];
    pfcp_xact_stage_t stage;
    ...
    stage = pfcp_xact_get_stage(type, xact->xid);
    if (xact->org == PFCP_LOCAL_ORIGINATOR)
    {
        switch(stage)
        {
...
status_t pfcp_xact_commit(pfcp_xact_t *xact)
{
    status_t rv;
    char buf[CORE_ADDRSTRLEN];
    ...
    rv = pfcp_send(xact->gnode, pkbuf);
    d_assert(rv == CORE_OK, return CORE_ERROR, "pfcp_send error");

    return CORE_OK;
}

status_t pfcp_xact_timeout(index_t index, c_uintptr_t event, c_uint8_t *type)
{
    char buf[CORE_ADDRSTRLEN];
    pfcp_xact_t *xact = NULL;
    
    d_assert(index, goto out, "Invalid Index");
    xact = index_find(&pfcp_xact_pool, index);
    d_assert(xact, goto out, "Null param");
    d_assert(xact->gnode, goto out, "Null param");

    d_assert(type, goto out, "Null param");
    *type = xact->seq[xact->step-1].type;
    ...
    if (event == g_response_event)
    {
        ...
        if (--xact->response_rcount > 0)
        {
            ...
            d_assert(pfcp_send(xact->gnode, pkbuf) == CORE_OK,
                    goto out, "pfcp_send error");
        }
        else
        {
...
status_t pfcp_xact_receive(
        pfcp_node_t *gnode, pfcp_header_t *h, pfcp_xact_t **xact)
{
    char buf[CORE_ADDRSTRLEN];
    status_t rv;
    pfcp_xact_t *new = NULL;

    d_assert(gnode, return CORE_ERROR, "Null param");
    d_assert(h, return CORE_ERROR, "Null param");

    new = pfcp_xact_find_by_xid(gnode, h->type, PFCP_SQN_TO_XID(h->sqn));
    if (!new)
        new = pfcp_xact_remote_create(gnode, h->sqn);
    d_assert(new, return CORE_ERROR, "Null param");

    d_trace(15, "[%d] %s Receive peer [%s]:%d\n",
            new->xid,
            new->org == PFCP_LOCAL_ORIGINATOR ? "LOCAL " : "REMOTE",
            CORE_ADDR(sock_remote_addr(gnode->sock), buf),
            CORE_PORT(sock_remote_addr(gnode->sock)));

    rv = pfcp_xact_update_rx(new, h->type);
    if (rv != CORE_OK)
    {
        return rv;
    }
...
pfcp_xact_t *pfcp_xact_find(index_t index)
{
    d_assert(index, return NULL, "Invalid Index");
    return index_find(&pfcp_xact_pool, index);
}

static pfcp_xact_stage_t pfcp_xact_get_stage(c_uint8_t type, c_uint32_t xid)
{
    pfcp_xact_stage_t stage = PFCP_XACT_UNKNOWN_STAGE;
    ...
    switch(type)
    {
        ...
        default:
            d_error("Not implemented PFCP Message Type(%d)", type);
            break;
    }
...
pfcp_xact_t *pfcp_xact_find_by_xid(
        pfcp_node_t *gnode, c_uint8_t type, c_uint32_t xid)
{
    ...
    switch(pfcp_xact_get_stage(type, xid))
    {
        case PFCP_XACT_INITIAL_STAGE:
            xact = list_first(&gnode->remote_list);
            break;
...
void pfcp_xact_associate(pfcp_xact_t *xact1, pfcp_xact_t *xact2)
{
...
void pfcp_xact_deassociate(pfcp_xact_t *xact1, pfcp_xact_t *xact2)
{
    d_assert(xact1, return, "Null param");
    d_assert(xact2, return, "Null param");
...
static status_t pfcp_xact_delete(pfcp_xact_t *xact)
{
    ...
    if (xact->assoc_xact)
        pfcp_xact_deassociate(xact, xact->assoc_xact);

    list_remove(xact->org == PFCP_LOCAL_ORIGINATOR ? &xact->gnode->local_list :
            &xact->gnode->remote_list, xact);
    index_free(&pfcp_xact_pool, xact);
